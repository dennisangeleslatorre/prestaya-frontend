...
import { getTipoProductoByCodigoTipoProducto, updateTipoProducto, registerTipoProducto } from '../../Api/Api'
...
const TipoProductoForm = (props) => {
    ...
    const buttonTypes = {
        nuevoTipoProduto: {label:"Guardar", class:"btn btn-primary btn-form"},
        editarTipoProduto: {label:"Actualizar", class:"btn btn-warning btn-form"},
        visualizarTipoProduto: {label:"Ir a lista", class:"btn btn-info btn-form"}
    }
    const readOnlyView = urlFragment === "visualizarTipoProduto" ? true : false;
    const readOnlyCode = urlFragment !== "nuevoTipoProduto" ? true : false;

    const formFunctions = {
        nuevoTipoProduto: ()=> handleRegister(),
        editarTipoProduto: ()=> handleUpdate()
    }

    const prepareNotificationSuccess = (message) => {
        setIsAlert(true);
        setNotification({title:"Operación exitosa", type:"alert-success", message:message});
        setResponseData({message: message, title: "Operación exitosa", url:"/tiposProducto"});
    ...
    const handleRegister = async () => {
        ...
        const response = await registerTipoProducto(data);
    ...
    const handleUpdate = async () => {
        ...
        const response = await updateTipoProducto({body:data, id:elementId});
    ...
    const handleClick = () => {
        if(urlFragment !== "visualizarTipoProduto") {
            if(validate()) {
                setOpen(true);
                if(urlFragment === "nuevoTipoProduto") setModalAttributes({title:"Aviso de creación", message:"¿Seguro que desea crear este elemento?"});
                if(urlFragment === "editarTipoProduto") setModalAttributes({title:"Aviso de actualización", message:"¿Seguro que desea actualizar este elemento?"});
            } else {
    ...
    const getData = async () => {
        const response = await getTipoProductoByCodigoTipoProducto(elementId);
    ...
    useEffect(async () => {
        ...
        if(urlFragment !== "nuevoTipoProduto") await getData();
        setIsLoading(false);
    }, [])
    ...
    return (
        <>
            ...
            goList={()=>history.push("/tiposProducto")} view={readOnlyView}>
                <InputComponent
                    state={tipoProducto}
                    setState={setTipoProducto}
                    type="text"
                    label="Tipo producto"
                    placeholder="Tipo producto"
                    inputId="tipoProductoInput"
