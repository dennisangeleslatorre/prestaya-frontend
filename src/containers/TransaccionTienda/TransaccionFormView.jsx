...
const TransaccionFormView = () => {
    ...
    const getData = async () => {
        ...
        if(responseCabecera.status === 200 && responseCabecera.body && responseCabecera.body.data) {
            ...
            setTipoDocumento(data.c_tipodocumento === "NI" ? "NOTA INGRESO" : "NOTA SALIDA");
            setEstado(data.c_estado === "RE" ? "REGISTRADO" : "ANULADO");
            setFechaDocumento({value: moment(data.d_fechadocumento).format("YYYY-MM-DD")});
            ...
            setResponseData({title: "Aviso", message: responseCabecera.body.message || "Ocurrio un problema con el servicio"});
    ...
    const getDetalles = async () => {
        ...
        if(responseDetalle.status === 200 && responseDetalle.body && responseDetalle.body.data) {
            getDataTable(responseDetalle.body.data);

        } else {
            setResponseData({title: "Aviso", message: responseDetalle.body.message || "Ocurrio un problema con el servicio"});
            setOpenResponseModal(true);
        }
    }
    ...
    const getDataTable = (detalles) => {
        ...
        const detallesAux = detalles.map( (item, index) => {
            ...
            aux.d_fecharegistro = moment(item.d_fecharegistro).format("DD/MM/yyyy HH:MM:ss");
            aux.d_ultimafechamodificacion = moment(item.d_ultimafechamodificacion).format("DD/MM/yyyy HH:MM:ss");
            total = Number(Number(total) + (Number(item.n_cantidad) * Number(item.n_precio))).toFixed(2);
            return aux;
        })
  ...
  return (
    <>
      <FormContainer buttonAttributes={{label:"Ir a lista", class:"btn btn-info btn-form"}} handleClick={handleClick}
        showButton={false} goList={()=>history.push(`/transacionestienda`)}
        view={false}>
