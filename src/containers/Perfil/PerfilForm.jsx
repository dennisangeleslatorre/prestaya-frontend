...
const PerfilForm = (props) => {
  ...
  const validate = () => {
    if (!perfil.isValid || !codigo.isValid || paginas.length === 0)
      return false;
    return true;
  };
  ...
  const handleRegister = async () => {
    ...
    } else prepareNotificationDanger("Error al registrar", response.message);
    setIsLoading(false);
  };
  ...
  const handleUpdate = async () => {
    ...
    if (response && response.status === 200) {
      ...
      assignReportesToProfile();
    } else prepareNotificationDanger("Error al actualizar", response.message);
    setIsLoading(false);
  };
  ...
  const getReportesByPerfilFunction = async () => {
    ...
    if (response.status === 200) {
      const data = response.body.data;
      let reportesAux = data
        .filter(
          (item) => item.c_tiporeporte === "CO000001" && item.c_acceso === "S"
        )
        .map(
          (reporte) =>
            `${reporte.c_tiporeporte}-${reporte.n_grupo}-${reporte.n_reporte}`
        );
