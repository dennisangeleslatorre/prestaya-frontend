...
const SearchModalProducto = (props) => {
    ...
    const tableColumnsProducto = [
      ...
      {name:'c_descripcion_tipo_producto', label: 'Tipo', sortVar:0},
...
const getProductosToTable = (productos) => {
    const listProductosTable = productos.map((item) => {
        ...
        aux.c_descripcion_tipo_producto = item.c_descripcion_tipo_producto;
        aux.c_descripcionproducto = item.c_descripcionproducto;
        aux.actions = <div className="row" style={{justifyContent: 'space-evenly'}}>
            <button type="button" onClick={() => handleSelectProducto(item)} className="btn btn-info"><i className="bi bi-check"></i></button>
        </div>
        return aux;
    })
...
useEffect(() => {
  ...
  return (
    <Modal isOpen={isOpen} title="Buscador de productos" onClose={onClose} modal_class="Modal__container__search">
            {/*body*/}
            <div className="modal-body">
                ...
                >
                    { filterProducto.length < 1 && <div className="invalid__message__data">
