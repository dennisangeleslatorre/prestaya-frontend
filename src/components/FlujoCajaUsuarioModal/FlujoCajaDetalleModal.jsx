...
import { getFlujoCajaDiasByCodigo} from '../../Api/Api'
...
const FlujoCajaDetalleModal = (props) => {
    const {isOpen, onClose, c_compania, n_correlativo, handleMostrarDetalleCajaMovimiento } = props;
    ...
    const setDataFlujoCajaDetalleTable = (detallesService) => {
        const listAux = JSON.parse(JSON.stringify(detallesService)).map((item, index) => {
            let aux = {};
            aux.key = index+1;
            aux.d_fechamov_format = item.d_fechamov ? moment(item.d_fechamov).local().format('DD/MM/yyyy') : "";
            aux.c_observaciones = item.c_observaciones;
            aux.c_estado = item.c_estado === "A" ? "ACTIVO" : "INACTIVO";
            aux.handleMostrarMovimientoDetalleRow = () => handleMostrarDetalleCajaMovimiento(c_compania, n_correlativo, moment(item.d_fechamov).local().format('yyyy-MM-DD'));
            return aux;
        });
        setDetalles(listAux);
    }
